...
if(strlen($_SESSION['alogin'])==0) {
    ...
    if(isset($_POST['submit'])) {
        ...
        // 獲取電單車類型和交易次數 (確保從 $_POST 獲取)
        $biketype=isset($_POST['biketype']) ? $_POST['biketype'] : ''; // 確保獲取 BikeType
        ...
        $sql="UPDATE tblvehicles SET
                    ...
                    VehiclesBrand=:brand,
                    BikeType=:biketype,               -- 新增 BikeType
                    VehiclesOverview=:vehicleoverview,
                    PricePerDay=:priceperday,
                    FuelType=:fueltype,
                    EngineDisplacement=:enginedisplacement, -- 新增 EngineDisplacement
                    ModelYear=:modelyear,
                    SeatingCapacity=:seatingcapacity,
                    TransactionCount=:transactioncount, -- 新增 TransactionCount
        ...
        $query->bindParam(':biketype',$biketype,PDO::PARAM_STR);               // 綁定 BikeType
        $query->bindParam(':vehicleoverview',$vehicleoverview,PDO::PARAM_STR);
        $query->bindParam(':priceperday',$priceperday,PDO::PARAM_STR); // 數據庫中是 INT，但如果允許小數，最好用 DECIMAL 或 FLOAT
        $query->bindParam(':fueltype',$fueltype,PDO::PARAM_STR);
        $query->bindParam(':enginedisplacement',$enginedisplacement,PDO::PARAM_INT); -- 綁定 EngineDisplacement
        ...
        $query->bindParam(':transactioncount',$transactioncount,PDO::PARAM_INT); -- 綁定 TransactionCount
